...
import { createZaviFishAudio, type ZaviFishAudio } from "./zavi-fish-audio";
...
export function ZaviFishGame({ playerDisplayName }: { playerDisplayName: string }) {
  ...
  const canvasRef = useRef<HTMLCanvasElement>(null); const stateRef = useRef(state); const holdingRef = useRef(false); const submittedRef = useRef(false); const audioRef = useRef<ZaviFishAudio | null>(null); const [muted, setMuted] = useState(false); const [saveMessage, setSaveMessage] = useState("");
  ...
  useEffect(() => { audioRef.current = createZaviFishAudio(); return () => { audioRef.current?.destroy(); audioRef.current = null; }; }, []);
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => { accumulated += Math.min(.1, (now - last) / 1000); last = now; while (accumulated >= 1 / 60) { const previous = stateRef.current; stateRef.current = stepFishGame(previous, holdingRef.current); if (previous.hook.caughtFishId === null && stateRef.current.hook.caughtFishId !== null) audioRef.current?.playCatch(); if (previous.score !== stateRef.current.score) audioRef.current?.playScore(); if (previous.status !== "Miss — reeling in." && stateRef.current.status === "Miss — reeling in.") audioRef.current?.playMiss(); if (previous.phase !== "over" && stateRef.current.phase === "over") { audioRef.current?.stopMusic(); audioRef.current?.playGameOver(); } if (stateRef.current.phase === "playing" && stateRef.current.hook.y !== previous.hook.y) audioRef.current?.playReel(stateRef.current.hook.y > previous.hook.y ? "down" : "up"); accumulated -= 1 / 60; } drawGame(context, stateRef.current); setState(stateRef.current); frame = requestAnimationFrame(tick); };
    frame = requestAnimationFrame(tick); return () => { cancelAnimationFrame(frame); observer.disconnect(); };
  }, []);
  useEffect(() => { const keyDown = (event: KeyboardEvent) => { if (event.code !== "ArrowDown") return; event.preventDefault(); const wasReady = stateRef.current.phase === "ready"; stateRef.current = startFishGame(stateRef.current); if (wasReady) { audioRef.current?.startMusic(); audioRef.current?.playCast(); } holdingRef.current = true; }; const keyUp = (event: KeyboardEvent) => { if (event.code === "ArrowDown") { event.preventDefault(); holdingRef.current = false; } }; window.addEventListener("keydown", keyDown); window.addEventListener("keyup", keyUp); return () => { window.removeEventListener("keydown", keyDown); window.removeEventListener("keyup", keyUp); }; }, []);
  useEffect(() => {
