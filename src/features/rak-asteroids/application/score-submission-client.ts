type ScoreSubmissionResponse = { error?: unknown; scoreId?: unknown };

export async function submitRakAsteroidsScore(score: number, submissionId: string): Promise<number> {
  const response = await fetch("/api/games/rak-asteroids/scores", {
    method: "POST",
  ...
  if (typeof body.scoreId !== "number") throw new Error("The score service returned an unexpected response.");
  return body.scoreId;
}
