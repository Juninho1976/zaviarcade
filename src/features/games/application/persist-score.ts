...
export async function persistScore(database: Database, slug: string, userId: string, submission: ScoreSubmission) {
  ...
  await database
    .prepare("INSERT OR IGNORE INTO scores (game_id, user_id, score, submission_id) VALUES (?, ?, ?, ?)")
    .bind(gameId, userId, submission.score, submission.submissionId)
    .run();
